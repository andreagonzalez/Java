...
//Atributos
 ...
 /*Este método foi criadao apenas para mostrar os resultados do OBJETO */   
...
//Métodos Personalizados
    ...
    public void fecharConta() {
        if (this.getSaldo() > 0) {
            ...
            System.out.println("Conta não pode ser fechada, saldo devedor");
        } else {
    ...
    public void depositar(float v) {
        if (this.getStatus()) {
            //this.saldo = this.saldo + v; //mexe direto nos atributos
            this.setSaldo(this.getSaldo() + v);//atualiza o saldo
    ...
    public void sacar(float v) {
        if (this.getStatus()) {
            if (this.getSaldo() >= v) {
                this.setSaldo(this.getSaldo() - v);//trabalhando com métodos
...
//o prof. usou só n como nome de numConta
